...
export function CategoryCard(category) {
    ...
    // Image or fallback
    let imgSrc = '';
    if (typeof category.image === 'string' && category.image) imgSrc = category.image;
    else if (category.image && typeof category.image === 'object') imgSrc = category.image.url || category.image.src || '';
    ...
    name.className = 'mt-3 text-sm font-medium text-gray-900 group-hover:text-primary-600 transition-colors text-center';
    name.textContent = category.name;
    card.appendChild(name);

    if (category.product_count) {
        const count = document.createElement('p');
        count.className = 'text-xs text-gray-500 text-center';
